...
Библиотека предназначена для управления документами в формате OpenXML
- чтение
- редактирование
- генерация
...
- В местах, куда требуется вставка данных в шаблоне создать "поля"
  ...
  - На вкладке "Разработчик" нажать кнопку "Aa" - "Элемент управления содержимым - Обычный текст"
...
После подготовки шаблона в коде заполнение выполняется следующим образом
...
foreach(var data in Excel.File("file.xlsx")["SheetName"].Skip(1))
{
  ...
  continue; // если строка не соответствует шаблону, то переходим к следующей

  if(!int.TryParse(id_str, out var id))
